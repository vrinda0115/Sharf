...
def slide_images(data: dict) -> list[Image.Image]:
    ...
    image, draw = shell(
        ...
        "A three-screen founder dashboard built on validated synthetic startup-month data and Monte Carlo risk forecasts.",
    )
    ...
    image, draw = shell(
        "Screen 3: Monthly log",
        "The founder can enter this month's values and receive a plain-language readout on whether the startup is improving.",
    )
    draw.rounded_rectangle((292, 180, 650, 600), radius=8, fill="#ffffff", outline="#d9ded9", width=2)
    draw.text((320, 210), "Monthly inputs", fill="#19201d", font=font(28, True))
    monthly_fields = ["Revenue", "Cash balance", "Monthly burn", "Monthly churn", "CAC payback", "NPS"]
    for index, field in enumerate(monthly_fields):
        y = 268 + index * 48
        draw.text((326, y), field, fill="#68736f", font=LABEL)
        draw.rounded_rectangle((472, y - 8, 618, y + 26), radius=6, fill="#f4f6f3", outline="#d9ded9", width=1)
    draw.rounded_rectangle((700, 180, 1160, 600), radius=8, fill="#ffffff", outline="#d9ded9", width=2)
    draw.text((728, 210), "Plain-language readout", fill="#19201d", font=font(28, True))
    draw_text_box(draw, (728, 276), "Overall, the startup appears to be improving because the health score increased and runway did not get worse.", "#19201d", 24)
    draw_text_box(draw, (728, 394), "The app also explains the burn decision, fundraising readiness, and weakest area to fix next.", "#43504b", 22)
    slides.append(image)
    ...
    image, draw = shell(
        ...
        "Open the prototype, scan portfolio health, pick a startup, then log a monthly update to see whether the startup is improving.",
    )
    steps = [
        ...
        "5. Use Monthly log to enter new values and get improvement feedback",
    ]
